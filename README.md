...
| 22| Убрал preload banner'а                                          |     544.08     |

*Погрешность проверки


* Решил оставить варианты изображений, исходный код и другой мусор в проекте, ведь задание не на чистоту проекта.
* Финальный результат 544.08 (он же 550.79 - когда у сервера было хорошее настроение). Уперся в оптимизацию шрифтов, использовал fontFaceObserver, который не дал улучшения производительности. Можно было бы минифицировать шрифт, т.к. он используется для кириллицы в одном случае и для чисел в другом. Но это будет работать, пока не надо будет добавить другие символы (какой смысл в такой оптимизации).
* Постарался сделать оптимизацию не для чисел, а для будущего поддержания проекта.